...
export function handleBackground(background?: string, dim = false, opacity = 0.5): CSSProperties {
  ...
  const overlayColorStart = `rgba(0, 0, 0, ${opacity * 1.0})`; // Start color of the gradient
  const overlayColorMid = `rgba(0, 0, 0, ${opacity * 0.95})`; // End color of the gradient
  const overlayColorEnd = `rgba(0, 0, 0, ${opacity * 0.85})`; // End color of the gradient
